...
// ==================== دوال الحفظ المزدوج (localStorage + Supabase) ====================
function saveAllData() {
    localStorage.setItem('users', JSON.stringify(users));
    localStorage.setItem('tasks', JSON.stringify(tasks));
    localStorage.setItem('messages', JSON.stringify(messages));
    localStorage.setItem('notifications', JSON.stringify(notifications));
    // ✅ حفظ في Supabase أيضاً للمزامنة
    syncDataToSupabase();
}

async function syncDataToSupabase() {
    try {
        await supabaseClient.from('users').upsert(users.map(u => ({
            id: u.id,
            name: u.name,
            password: u.password,
        ...
        await supabaseClient.from('tasks').upsert(tasks.map(t => ({
            id: t.id,
            title: t.title,
            refs: t.refs,
        ...
        await supabaseClient.from('messages').upsert(messages.map(m => ({
            id: m.id,
        ...
        console.log('✅ تم المزامنة مع Supabase');
    } catch (error) {
        console.log('⚠️ المزامنة المحلية فقط:', error.message);
    }
}

async function loadDataFromSupabase() {
    try {
        const {  usersData } = await supabaseClient.from('users').select('*');
        const {  tasksData } = await supabaseClient.from('tasks').select('*');
        const {  messagesData } = await supabaseClient.from('messages').select('*');
        
        if (usersData && usersData.length > 0) {
            users = usersData.map(u => ({
                ...u,
                isAdmin: u.is_admin,
                tasks: users.find(local => local.id === u.id)?.tasks || [],
                notifications: users.find(local => local.id === u.id)?.notifications || [],
                chatPermissions: users.find(local => local.id === u.id)?.chatPermissions || []
            }));
            localStorage.setItem('users', JSON.stringify(users));
        }
        ...
        if (tasksData && tasksData.length > 0) {
            tasks = tasksData.map(t => ({
                ...t,
                files: tasks.find(local => local.id === t.id)?.files || []
            }));
            localStorage.setItem('tasks', JSON.stringify(tasks));
        }
        ...
        console.log('⚠️ استخدام البيانات المحلية:', error.message);
    }
}

// ==================== دوال الإشعارات (نفس الدوال الأصلية) ====================
...
// ==================== دوال رفع الملفات (نفس الدوال الأصلية) ====================
...
// ==================== دوال صلاحيات المحادثة (نفس الدوال الأصلية) ====================
...
// ==================== دوال تغيير الرقم السري (نفس الدوال الأصلية) ====================
...
// ==================== دوال الرسائل الجماعية (نفس الدوال الأصلية) ====================
...
// ==================== إدارة الصفحات (نفس الدوال الأصلية) ====================
const app = document.getElementById('app');
...
// ==================== دوال المستخدمين (نفس الدوال الأصلية) ====================
...
// ==================== دوال المهام (نفس الدوال الأصلية) ====================
...
// ==================== عرض صفحات الموقع (نفس الدوال الأصلية) ====================
    ...
    app.innerHTML = `
        <div class="login-wrapper">
            <div class="login-glass">
                ...
                <button class="accounts-button" onclick="window.open('', '_blank')">
                    <span class="btn-icon">👥</span>
                    الحسابات
                </button>
...
// ==================== دوال المعالجة (نفس الدوال الأصلية) ====================
